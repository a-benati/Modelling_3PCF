...
void cbl::modelling::threept::Modelling_ThreePointCorrelation_comoving_connected::set_model_RSD_all_scales (const statistics::PriorDistribution b1_prior, const statistics::PriorDistribution b2_prior, const statistics::PriorDistribution bt_prior, const statistics::PriorDistribution beta_prior)
{
  // set the model parameters
  const int nparameters = 4;
  ...
  // construct the model
  m_model = make_shared<statistics::Model1D>(statistics::Model1D(&zeta_RSD_all_scales, nparameters, parameterType, parameterName, inputs));
}
